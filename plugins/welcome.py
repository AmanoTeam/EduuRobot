...
async def set_welcome_message(c: Client, m: Message, strings):
    if len(m.text.split()) > 1:
        ...
        await m.reply_text(strings("welcome_set_success").format(chat_title=m.chat.title))
    else:
       ...
       pass
        ...
        if welcome[1]:
            if welcome[0] is not None:
                ...
                welcome = strings("welcome_default").format(user_name=escape_markdown(first_name),
