...
async def ip_cmd(c: Client, m: Message, s):
    ...
    if not ips:
        await m.reply_text(s("ip_err_no_ips").format(domain=text))
        return

    if len(ips) == 1:
        await m.reply_text(format_api_return(await get_api_return(ips[0]), s))
        return

    await m.reply_text(
        s("ip_select_ip").format(domain=text),
...
async def ip_inline(c: Client, q: InlineQuery, s):
        ...
        await q.answer([
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    s("ip_no_url_example").format(bot_username=c.me.username),
                ),
            )
        ])
        return
    ...
    if not ips:
        articles = [
            InlineQueryResultArticle(
                title=s("ip_err_no_ips").format(domain=text),
                input_message_content=InputTextMessageContent(
                    s("ip_err_no_ips").format(domain=text),
                ),
            )
        ]
        ...
        articles = [
            InlineQueryResultArticle(
                title=s("ip_inline_err_bogon_ip").format(ip=ips[0])
                if api_return.get("bogon")
                else s("ip_info_inline").format(domain=text),
                input_message_content=InputTextMessageContent(
                    format_api_return(api_return, s),
                ),
            )
        ]
    else:
        articles = [
            InlineQueryResultArticle(
                title=s("ip_select_ip").format(domain=text),
                input_message_content=InputTextMessageContent(
                    s("ip_select_ip").format(domain=text),
                ),
                reply_markup=InlineKeyboardMarkup([
        ...
        articles.extend([
            InlineQueryResultArticle(
                title=ip,
                input_message_content=InputTextMessageContent(
                    format_api_return(await get_api_return(ip), s),
                ),
            )
            for ip in ips
        ])
